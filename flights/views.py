...
def search(request):
    ...
    if form.is_valid():
       ...
       # destination = form.cleaned_data['destination']
        ...
        flights = Flight.objects.filter(origin__exact=origin, destination__exact=destination)
...
def flight_search(request, origin, destination):
    ...
    flights = Flight.objects.filter(origin__exact=origin).filter(destination__exact=destination)
